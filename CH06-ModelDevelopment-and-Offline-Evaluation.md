...
- 만들어진 ML 모델이 좋은지, 나쁜지 어떻게 알 수 있을까?    
- ML 시스템 평가 방법을 명확히 이해하지 못했다고 해서 ML 프로젝트가 반드시 실패하는 것은 아니지만    
  적합한 솔루션을 찾을 수 없게 되고 관리자가 ML을 채택하도록 설득하기 또한 한층 어려워짐.   
- 비즈니스 팀과 협력해 회사 비즈니스와 더 밀접하게 관련된 모델 평가 지표를 개발하기도 함   

- 이상적인 평가 방법은 개발과 프로덕션 환경 간에 동일해야 함   
  보통 개발 환경에서는 그라운드 트루스 레이블이 있지만 프로덕션 환경에서는 현실적으로 실현하기 어려움   

- 특정 작업에서는 '자연 레이블'에서 설명한 대로 사용자 피드백을 기반으로 프로덕션 환경 레이블을   
  유추하거나 대략적인 값을 지정할 수 있음   
  추천 작업이라면 사용자가 클릭했는지에 따라 추천 항목이 좋은지 유추함.그러나 상당한 평향이 발생함   

- 다른 작업에서는 프로덕션 환경에서 모델 성능을 직접 평가하기 어려움  
  ML 시스템 성능의 변화와 오류를 탐지하기 위해 광범위한 모니터링을 수행해야 함.    


- 배포 후에는 프로덕션 환경에서 모델을 계속 모니터링하고 테스트해야 함.   


#### 6.2.1 베이스라인    

- 평가 지표는 그 자체로는 큰 의미가 없음.    
  모델을 평가할 때는 평가 기준을 정확히 알아야하고, 정확한 베이스라인은 유스 케이스마다 다르지만   
  대부분의 경우에 유용한 베이스라인은 다음과 같음    



##### <무작위 베이스라인>    

- 모델이 무작위로 예측한다면 기대 성능이 얼마일까?     
  예측은 균등 분포나 작업의 레이블 분포 등 특정 분포에 따라 무작위로 생성함    

  ex, 레이블이 두 개인 작업을 가정할 때 90% 빈도로 나타나는 음성 클래스와 10%    
  빈도로 나타나는 양성 클래스가 있음. 무작위로 예측하는 베이스라인 모델의 F1 점수와 정확도는   

  균등 무작위 : 각 레이블을 동일한 확률(50%)로 예측 => F1 0.167, ACC 0.5   
  작업의 레이블 분포 90% 음성, 10%는 양성으로 예측 => F1 0.1 ACC 0.82   

##### [단순 휴리스틱]    

- 단순 휴리스틱 기반으로 예측한다면?   
  사용자가 뉴스 피드에 머무는 시간을 늘리기 위해 뉴스 피드 항목에 순위를 매기는 시스템을 구축한다고 가정할 때,   
  순위를 역시간순으로 매겨 쵯니 항목을 가장 먼저 보여준다면 사용자 체류 시간이 얼마나 될까?    

##### [0 규칙 베이스라인]     

- 단순 휴리스틱 베이스라인의 특수한 경우로 베이스라인 모델이 가장 흔한 클래스로 예측함    
  ex, 사용자가 휴대 전화에서 다음 차례에 사용할 확률이 가장 높은 앱을 추천한다면   
  단순한 모델은 가장 자주 사용하는 앱을 추천함.    
  이 단순한 휴리스틱이 70%의 확률로 다음 차례의 앱을 정확히 예측한다면 개발하는 모델은 복잡도가 커지는 대신    
  베이스라인 모델 성능을 크게 능가해야 함.    



##### [인간에 의한 베이스라인]     

- ML은 인간이 수행하는 작업을 자동화하는 것을 목표로 할 때가 많음    
  전문가와 비교했을 때 모델이 성능이 어떤지 파악함   
  ex, 자율 주행 시스템을 인간 운전자와 비교에 진척도를 측정함.   
  목적이 전문가를 대체하려는 것이 아닌 생산성을 향상하는 것이더라도 해당 시스템이 어떤 시나리오에서   
  인간에게 유용한지 아는 것은 중요함   
...
- 많은 경우 ML 시스템은 다수 if/else 문 또는 서드 파티 솔루션을 적용한    
  기존 비즈니스 로직 솔루션을 대체하도록 설계됨    

- 새 모델을 기존 솔루션과 비교해보는 일이 중요함     

- 모델을 평가할 때 '좋은 시스템'과 '유용한 시스템'을 구별하는 것이 중요함.     
  좋은 시스템이 반드시 유용한 것이 아니며 나쁜 시스템이 반드시 유용하지 않은 것도 아님.     
  자율 주행 자동차는 이전 자율 주행 시스템보다 크게 개선된다면 좋은 것이지만 사람 운전자만큼 성능이 나오지 않으면 유용하지 않다.    
  경우에 따라 ML 시스템 성능이 사람의 평균치보다 높더라도 사람들이 이를 신뢰하지 않아 유용하지 않기도 함    
  사용자가 휴대 전화에서 다음에 입력할 단어를 예측하는 시스템이 해당 언어를 사용하는 사람의 예측보다   
  성능이 훨씬 낮다면 나쁜 것으로 간주될 수 있음. 다만, 예측이 때로 사용자가 더 빠르게 입력하는데 도움을 준다면 유용함      
...
- 학문의 맥락에서 ML 모델을 평가할 때는 주로 성능 지표가 중심이 됨.  

- 하지만 프로덕션 환경에서는 모델이 강건하고, 공정하고, 잘 보정되고, 개연성 있는 결과를 내놓기를 원함.   


##### [교란 테스트]     

- 이상적으로 모델 개발에 사용하는 입력값이 프로덕션 환경에서 다뤄야 하는 입력값과 유사하지만 이는 대부분 현실적으로 어려움.    

- 데이터 수집이 어렵거나 비용이 들고 훈련으로 접근 가능한 데이터가 최상의 조건임에도 불구하고 실제 데이터와는 크게 다른 경우임     

- 모델이 프로덕션 환경에서 다뤄야 하는 입력값은 개발 환경 입력값 대비 잡음이 많음.    
  훈련 데이터에서 가장 잘 동작하는 모델과 잡음이 있는 데이터에서 가장 잘 동작하는 모델이 항상 같지는 않음     
- 데이터에 잡음이 있을 때 모델이 얼마나 잘 작동하는지 알아보는 방법으로 테스트 분할에 약간 변화를 줘서 모델 성능에 어떤 영향을 미치는지 측정해봐도 좋음      
  모델이 잡음에 민감할수록 유지 관리가 어려워지므로, 모델이 적대적 공격에 취약해지기도 함     


##### [불변성 테스트]    

- 입력에 특정 변경을 적용했을 때 출력이 변하면 안됨.   
  이러한 경우 모델에 편향이 존재하므로 성능이 좋아도 사용할 수 없음    

- 편향을 피하는 해결책으로 나머지 입력을 동일하게 유지하고 민감 정보만 변경해 출력이 바뀌는지 확인해보며,   
  모델 훈련에 사용한 피처에서 민감 정보를 최우선으로 제외함    



##### [방향 예상 테스트]    

- 입력에 특정 변경을 적용했을 때 출력이 예측 가능한 방향으로 변해야함.
  
- 출력이 예상과 반대로 움직인다면 모델 학습이 올바르게 이뤄지지 못했을 수도 있으므로 배포전에 좀 더 조사함     


##### [모델 보정]    


- 어떤 사건이 70% 확률로 발생하리라 예측했다고 가정할 때, 이는 예측 대상이 되는 현실을 무한히 반복했을 때 예측 결과가 실제 결과와 70% 확률로 일치한 다는 것임      
  모델이 A팀이 B팀을 70% 확률로 이길 거라고 잘 예측했찌만, 1,000회 대결 중 A팀이 60%만 이긴다면 이 모델은 보정되지 않은 것임    
  잘 보정된 모델은 A팀이 60%로 승리할 것이라고 예측해야 함    

- 모델 보정은 ML 실무에서 간과되는 경우가 많지만 모든 예측 시스템에서 매우 중요한 속성임    

  Ex, 사용자가 다음에 볼 가능성이 높은 영화를 추천하는 시스템을 개발한다고 가정할 때, 사용자 A는 로맨스 영화 80%, 코미디 영화를 20%를 봄.    
  A가 볼 가능성이 가장 높은 영화를 정확도 높게 노출하려면 추천 시스템은 추천 항목을 로맨스 영화로만 구성해야 함.   
  A는 다른 장르보다 로맨스를 볼 가능성이 훨씬 높기 때문임. 한편 보다 보정된 시스템을 원한다면, 사용자의 실제 시청 습관이 드러나게 로맨스 80%, 코디미 20%으로 보정 해야 함     

- 보정된 정도를 간단히 측정하려면, 모델이 확률 X를 출력한 횟수와 예측이 맞은 빈도 Y를 세어보고 Y에 대한 X를 플롯해봄   
  모델이 완벽하게 보정됐다면 그래프상 모든 데이터 포인트에서 X와 Y가 동일함   
 
  sklearn.calibration.calibration_curve를 사용해 이진 분류기의 보정 곡선을 그릴 수 있음   
...
- 확실한 모델 예측 내용만 보여주고자 할 때, 그 확실성의 임곗값은?    
- 신뢰도 측정은 개별 샘플에 대한 지표로, 다른 지표들이 시스템 평균 성능을 측정하는 것과 다르다.     
  시스템 수준의 측정은 전체 성능을 파악하는 데 유용하지만, 각 샘플에 대한 시스템 성능을 고려한다면 샘플 수준의 지표가 중요함      


##### [슬라이스 기반 평가]      

- 슬라이싱은 데이터를 하위 집합으로 분리하고 각 하위 집합마다 모델의 개별 성능을 확인하는 것을 의미한다.

- 많은 회사에서 전반적인 f1 이나 전체 데이터에 대한 정확도 같이 투박한 지표에 집중해 슬라이스 기반 지표에 주의를 기울이지 않는다.

- 슬라이스 기반 지표에 주의를 기울이지 않으면     
  모델이 서로 다른 데이터 슬라이스에게 동일하게 수행돼야 할 때 다르게 수행 된다.    
  EX, 데이터에 하위 집단 두 개, 즉 다수와 소수가 있을 때 다수에 속한는 하위 집단이 데이터의 90%를 차지하고 있을 때    
  모델 A는 다수 하위 집단에서 98% 정확도, 소수 하위집단에서 80%으로 전체 정확도 96.2% / 모델 B는 다수에서 95%, 소수에서 95%로 전체 정확도는 95%이다.    
  회사가 전체 지표에 초점을 맞추면 모델 A를 선택하겠지만, 쵲오 사용자는 소수 하위집단이 과소 대표된 인구 통계 집단이므로 모델에 소수 하위 집단에 대한 편향이 존재함.    
  회사에서 두 모델의 슬라이스 기반 성능을 본다면, 소수하위 집단에 대한 모델 a의 성능을 개선하면 전반적으로 개선하는 결과로 이어짐   
  
- 두 번쨰 문제로 모델이 서로 다른 데이터 슬라이스에게 다르게 수행돼야 할 때도 동일하게 수행되는데, 데이터에서 일부 하위 집합은 다른 것보다 더 중요하다.   
  ex, 사용자 이탈 예측 모델 및 사용자가 구독이나 서비스 구입을 취소할지 예측하는 모델을 개발할 때 유료 사용자는 무료 사용자보다 훨씬 중요함.        
  모델의 전체 성능에 초점을 맞추면 중요 슬라이스에게 성능이 저하되더라도 모르고 넘어 갈 수 있음.       

- 슬라이스 기반 평가가 중요한 이유는 직관에 반하는 '심슨의 역설' 때문이다. 어떤 추세가 여러 데이터 집단에 나타나지만 집단을 결합하면 사라지거나 반전되는 현상이다.    
  모델 A => 집단 A에 대해 93%(81/87), 집단 B에 대해 73%(192/263), 전체 78%(273/350)  
  모델 B => 집단 A에 대해 87%(234/270), 집단 B에 대해 69%(55/80), 전체 83%(289/350)  

  즉, 모든 데이터를 함께 놓고 보면 모델 B가 모델 A보다 더 잘작동하지만, 각 부분 집단별로 개별적으로 놓고 보면 모델 A가 모델 B보다 더 잘 동작함   

- 집계가 실제 상황을 은폐하고 모순되게 할 수 있으므로, 정보를 입각해서 어떤 모델을 선택할지 결정 내릴 때는 전체 데이터뿐 아니라 개별 슬라이스에 대한 성능도 고려해야함     

- 슬라이스 기반 평가가 주는 통찰은 전체 및 중요 데이터에서 모델 성능을 개선하고 잠재적인 편향을 감지하는데 도움이 된다.      

- ML과 상관없는 문제가 존재한다는 사실도 밝혀내기도 함     

- 슬라이스 평가가 별문제 아닌 경우에도 모델이 동작하는 방식을 세분화해 이해한다면 상사, 고객 등 이해관계자가 ML 모델을 신뢰하도록   
  설득할 수 있는 모델에 대한 확신을 얻게 됨     

- 데이터에서 중요한 슬라이스를 찾는 방법은 집중적인 데이터 탐색과 분석이 필요한데, 주요 접근법은 아래의 세가지 이다.      




(1) 휴리스틱 기반   
: 데이터와 작업에 대한 도메인 지식 활용    
예를 들어 웹 트래픽 관련 작업에서 모바일 대 데스크톱, 브라우저 유형과 사용자 위치 같은 차원에 따라 데이터 슬라이싱    

(2) 오류 분석    
: 잘못 분류된 데이터 포인트를 수작업으로 분석하고 거기서 패턴을 찾음    

(3) 슬라이스 파인더    
: 빔 검색, 클러스터링, 의사 결정 같은 알고리즘으로 슬라이스 후보를 생성하고 명백히 나쁜 후보를 걸러내고 남은 후보의 순위를 지정함    


- 중요한 슬라이스를 찾았다면, 평가를 위해 슬라이스마다 레이블링된 데이터가 충분히 필요함을 알아야함 평가 품질은 데이터의 품질에 좌우됨     
...
- 작업에 가장 적합한 ML 모델을 선택하는 방법은 개발 모델 아키텍처의 장단점을 살펴보는 대신,    
  어떤 모델이 특정 목표/제약 조건/요구사항에 가장 적합한지를 감안해 결정함     

- 모델 개발 단계에서는 다양한 모델로 실험을 수행하고, 대량의 실험을 집중적으로 추적하고 버전을 관리하는 일은 중요함.     
  추적과 버전 관리 프로세스를 자동하기 위한 도구와 적절한 인프라를 갖추는 것이 필수임     
- 모델이 커지고 더 많은 데이터를 소비함에 따라 분산 훈련은 ML 모델 개발자에게 필수 기술이 되고 있음.     
  데이터 분산 시스템에서 매개변수가 수억 개가 달하는 모델을 동작시키는 것은 시스템 엔지니어링 전문 지식이 필요함      

- 배포에 가장 적합한 모델을 선택하기 위해 모델을 평가하는 방법은, 평가 지표에 비교할 기준이 없다면 큰 의미가 없으므로 평가에 고려할 만한 다양한 기준을 살펴봐야 함.     

- 프로덕션 환경에서 모델을 평가하기 앞서 모델이 온전한 상태인지 확인해야 함     

- 오프라인 모델 평가가 아무리 우수하더라도 모델 배포 전까지는 프로덕션 환경에서의 모델 성능을 확신할 수 없는 경우가 많음     
